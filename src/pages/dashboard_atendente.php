...
$var_search =  isset($_GET['search']) ? $_GET['search'] : "";
...
if ($var_search != '') {
    ...
    $sql_cod = "SELECT cod_paciente
                FROM pacientes
                WHERE nome_paciente = '$var_search'";

    $result_cod = $conn->query($sql_cod);

    if ($result_cod && $result_cod->num_rows > 0) {
        $codigo = $result_cod->fetch_assoc()['cod_paciente'];

        $sql_total = "SELECT COUNT(*) FROM `entregas` WHERE cod_paciente = '$codigo' ";
        $total_result = $conn->query($sql_total);
        $total_rows = $total_result->fetch_row()[0];
        // Calcular o número total de páginas
        $total_pages = ceil($total_rows / $items_per_page);

        $sql = "SELECT e.*, p.nome_paciente, p.cpf_paciente
                FROM entregas e
                LEFT JOIN pacientes p ON e.cod_paciente = p.cod_paciente
                WHERE e.cod_paciente = '$codigo'
                LIMIT {$items_per_page} OFFSET {$offset};";

    } else {
        // Tratamento se não encontrar o paciente
...
<div class="container">
                    ...
                    <form action="" method="">
                        ...
                        <div class="border-top border-secondary p-2">
                            ...
                            <select class="form-control rounded-4" name="nome" id="DropDownNome">
                                ...
                                <option value="<?php echo $row['cod_paciente'] ?>"><?php echo $row['nome_paciente'] ?>
                                </option>
                                <?php endwhile; ?>
                            </select>
    ...
    <div class="row">
        ...
        <div class="col-10">
            ...
            <!-- Botão que ativa o modal e barra de pesquisa alinhados -->
            <div class="row align-items-center">
                <!-- Botão "Cadastrar" alinhado à esquerda -->
                <div class="col-2">
                    <button type="button" style="background-color: #17a2b8; color: #FFF;" class="btn mt-3"
                        data-bs-toggle="modal" data-bs-target="#cadastromodal">
                        Cadastrar
                    </button>
                </div>

                <!-- Espaço flexível para empurrar a barra de pesquisa para a direita -->
                <div class="col-7"></div>

                <!-- Barra de pesquisa alinhada à direita -->
                <div class="col-3">
                    <div class="input-group rounded mt-3">
                        <input type="search" class="form-control rounded" id="search_bar_entrega"
                            placeholder="Search the name of pacients" aria-label="Search"
                            onkeyup="pesquisar_entrega(this.value)" aria-describedby="search-addon-entregas" />
                        <button class="input-group-text border-0" id="search-glass" onclick="searchEntregas()" required>
                            <i class="fas fa-search"></i>
                        </button>
                    </div>
                    <!-- Span para exibir os resultados da busca -->
                    <span class="input-group-text border-0" id="search-addon-entregas"></span>
                </div>
            </div>

            <style>
            #search-addon-entregas {
                position: relative;
                z-index: 1;
